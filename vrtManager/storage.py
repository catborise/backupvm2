...
class wvmStorage(wvmConnect):
    ...
    def get_allocation(self):
        return (util.get_xml_path(self._XMLDesc(0), "/pool/allocation"))
    ...
    def create_volume_winfo(self, image_info: dict):
        def __create_volume(name, backing_fullname, img_format="qcow2"):
            stpVolXml = """
             <volume>
               <name>""" + name + """</name>
               <allocation>0</allocation>
            ...
            return self.pool.createXML(stpVolXml, 0)

        if self.is_vol_exist(image_info["base"]) is not None and os.path.exists(image_info["base"]):
            if not self.is_vol_exist(image_info["name"]) is None:
                logging.error(image_info["name"] + " : belirtilen volume " + self.get_target_path() + " lokasyonunda zaten var. vol ismini kontrol ediniz.")
                return 0
            else:
                r = __create_volume(image_info["name"], image_info["base"])
                return 1
        else:
            logging.warning(image_info["base"] + " base volume could not found")
            return 0

    def delete_volume_with_info(self, image_info: dict):
        vol = self.get_volume(image_info["name"])
        if vol == None:
            logging.warning("Volume does not exist. Operation continues")
            return 0
        else:
            vol.delete(0)
            return 1